...
for jind in range(len(indsel)):
    ...
    if (jind > 0) & (np.mod(jind, 10) == 0):
        Pmm_ratio_all_samp_array = np.array(Pmm_ratio_all_samp)
        Pmin = np.percentile(Pmm_ratio_all_samp_array, 16, axis=0)
        Pmax = np.percentile(Pmm_ratio_all_samp_array, 84, axis=0)   
