...
analysis_dict['nz_lens_info_dict'] = {
...
# Disable theory beam smoothing — sim maps are not pre-smoothed by a theory
# beam; pixel beam effects are handled separately per map type below
...
try:
    ...
    # max_paint_R200c_factor=3.0. Gas beyond this radius is not painted in the
    # sim, so we zero it in theory too.
    # -------------------------------------------------------------------------
    ...
    # Gaussian approximation to the HEALPix pixel beam at nside=512.
    # Applied only to theory for maps painted with smooth profiles (gas, kappa).
    # Galaxy map is a point-source bincount map — no pixel beam applies.
    ...
    for label, th_arr, y_label, map_key in stats:
        ...
        th = interp1d(ell_theory, th_arr, bounds_error=False, fill_value=0.0)(l_range)
        ...
        idx = np.argmin(np.abs(l_range - 200))
        amp_ratio = (cl_sim[idx] / th[idx]
                     if np.abs(th[idx]) > 1e-30 and np.abs(cl_sim[idx]) > 1e-30
                     else np.nan)

        print(f"  {label}: RedChi2(100-500) = {chi2:.2e}, sim/theory at ell=200: {amp_ratio:.3f}")
    ...
    # ==========================================================================
    # 7. GENERATE STYLED 2x2 PLOT
    # ==========================================================================
    print("5. Generating styled 2x2 plot...")
    ...
    for i, ax in enumerate(axes):
        ...
        if data:
            ax.plot(l_range, data['th'],  lw=10, c='r', ls='-',        label='Theory')
            ax.plot(l_range, data['sim'], lw=10, c='k', ls='-', alpha=0.7, label='Sim')
            ax.set_ylabel(data['ylabel'])
            if i == 0:
                ax.legend(loc='upper right', fontsize=40, framealpha=1.0)
    ...
    plt.subplots_adjust(hspace=0.025, wspace=0.26)
    outfile = 'plots_tvss/multiplot_2pts.pdf'
