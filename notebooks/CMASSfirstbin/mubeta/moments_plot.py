...
for name in TRACERS:
    ...
    #np.random.choice(other_indices, 4, replace=False)
    ...
    for idx in random_indices:
        mb = theta_train[idx]
        plt.plot(SCALES, ratios[idx], alpha=0.5, lw=5, 
                 label=rf'$\mu_{{\beta}} = ${mb}') # No label for gray lines to keep legend clean

    # Plot the BEST match (thick blue line)
    #mb_best = theta_train[best_idx]
    #print("for ",name," best fit params are ", f"{mb_best:.2f}")
    #plt.plot(SCALES,ratios[best_idx],color='blue',marker='o',lw=3,markersize=8,
    #label=(rf'BEST MATCH: 'rf'\mu_\beta={mb_best:.2f}$'))
    # Add a dummy line for the "Other Samples" to the legend
    #plt.plot([], [], color='green', alpha=0.5, lw=5, label='5 Random Samples')
    ...
    plt.tight_layout()
    plt.savefig(f'plotsmubeta/moments_ratio_{name}.png', dpi=300)
    plt.show()

print("Plots generated. Look for 'plotsmubeta/moments_ratio_...png' files.")
