...
#SBATCH --output=/work/hdd/bdne/aacharya2/GODMAX/notebooks/CMASSfirstbin/new/run_scripts/logs/R3_%A_%a.out

# Configuration
NSIDE=512
TOTAL_SPLITS=1
WORK_DIR="/work/hdd/bdne/aacharya2/GODMAX/notebooks/CMASSfirstbin/new"
CSV_FILE="${WORK_DIR}/round3_samples.csv"
...
# We add 700 to the index to create unique folder names
FINAL_ID=$((SAMPLE_ID + 700))

echo "Task ${SLURM_ARRAY_TASK_ID}: R3 Sample Index ${SAMPLE_ID} (ID ${FINAL_ID}), Split ${JDEVICE} of ${TOTAL_SPLITS}"
