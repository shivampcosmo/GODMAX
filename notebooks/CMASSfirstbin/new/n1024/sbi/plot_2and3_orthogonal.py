...
def transform_samples(samples):
    """
    (theta_ej_0, nu_theta_ej_M) --> (theta_ej_0, theta_ej_0 * cbrt(nu_theta_ej_M))
    np.cbrt handles negative values correctly (real cube root).
    """
    p1 = samples[:, 0]
    p2 = p1 * np.cbrt(samples[:, 1])
...
# Labels: PlotSinglePosterior no longer used, only GetDist labels needed
labels_gd   = [r'\theta_{ej,0}',
               r'\theta_{ej,0}\,(\nu_{\theta_{ej}}^{M})^{1/3}']
param_names  = ['p1', 'p2']

# Truth values in the transformed space
truth_values = np.array([
    2.0,
    2.0 * np.cbrt(-0.1),   # approx -0.928
])
...
os.makedirs('plotcontours2params_orth', exist_ok=True)

# Limits in the transformed space.
# p2 range: theta_ej_0 in [1,6], cbrt(nu) in [cbrt(-0.3), 0] --> p2 in [6*cbrt(-0.3), 0]
#shifting to cbrt(-0.01) and -0.2, for clarity
PARAM_LIMITS = [
    (1.0,  6.0),                    # p1 : theta_ej_0
    (6.0 * np.cbrt(-0.01), -0.2),    # p2 : theta_ej_0 * cbrt(nu_theta_ej_M)  ≈ (-4.10, 0.0)
]


# =============================================================================
# 2. INDIVIDUAL POSTERIOR PLOTS — GetDist
# =============================================================================
...
for name in stat_map_keys:
    ...
    out_dir = Path(f'plotcontours2params_orth/{name}')
    out_dir.mkdir(exist_ok=True)

    smp = get_mcsamples(name, name)
    if smp is None:
        continue

    g = plots.get_subplot_plotter(width_inch=6, settings=plot_settings)
    g.triangle_plot(
        [smp],
        filled=True,
    ...
    for i in range(len(param_names)):
        ...
        g.subplots[i, i].axvline(truth_values[i], color='black', ls='--', lw=2)
        for j in range(i):
            lo_j, hi_j = PARAM_LIMITS[j]
            g.subplots[i, j].set_xlim(lo_j, hi_j)
            g.subplots[i, j].set_ylim(lo_i, hi_i)
            g.subplots[i, j].axvline(truth_values[j], color='black', ls='--', lw=2)
            g.subplots[i, j].axhline(truth_values[i], color='black', ls='--', lw=2)
...
# =============================================================================
# 3. GROUP COMPARISON PLOTS — GetDist
# =============================================================================
...
for group in tracer_groups:
    ...
    output_fn = f'plotcontours2params_orth/contour_comparison_{group["title"]}.png'
    g.export(output_fn)
