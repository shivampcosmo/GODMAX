...
# Keys stored inside the theory HMC npz as  samples_{name}        ← +mu_beta
THEORY_PARAM_NAMES = ['theta_ej_0', 'nu_theta_ej_M', 'nu_theta_ej_z', 'mu_beta']
THEORY_PRIOR_MIN   = np.array([1.0, -0.3, -3.0,  0.01])            # ← +0.01
THEORY_PRIOR_MAX   = np.array([6.0,  0.0,  3.0,  1.5 ])            # ← +1.5

# =============================================================================
# SHARED PARAMETER METADATA                                         ← +p4
# =============================================================================
...
TRUTH_VALUES = np.array([2.0, -0.1, 0.0, 0.5])                     # ← +0.5
PARAM_LIMITS = [(1.0, 6.0), (-0.3, 0.0), (-3.0, 3.0), (0.01, 1.5)]# ← +mu_beta

# =============================================================================
# PATHS
# =============================================================================
OUTPUT_DIR      = 'plotcontours_hmcvssbi_4params'                   # ← 4params
...
def load_hmc_mcsamples(probe, label='HMC / NUTS (sim)'):
    ...
    data = np.load(path)
    try:
        samples = np.column_stack([                                  # ← +mu_beta
            data['theta_ej_0'].ravel(),
            data['nu_theta_ej_M'].ravel(),
            data['nu_theta_ej_z'].ravel(),
            data['mu_beta'].ravel(),
        ])
    ...
    print(f'  [{probe}] Sim HMC: {samples.shape}  '
          ...
          f'nu_M={samples[:,1].mean():.3f}±{samples[:,1].std():.3f}  '
          f'nu_z={samples[:,2].mean():.3f}±{samples[:,2].std():.3f}  '
          f'mu_beta={samples[:,3].mean():.3f}±{samples[:,3].std():.3f}')  # ← new
...
for probe in PROBES:
    ...
    g = plots.get_subplot_plotter(width_inch=10, settings=plot_settings)  # ← 10 (was 8)
    g.triangle_plot(
        mcsamples_list,
        filled=True,
        colors=colors,
