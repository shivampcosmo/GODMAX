...
for probe_name in PROBES_TO_RUN:
    ...
    if RUN_SBI:
        run_sbi(
            ...
            ell_min=ELL_MIN,
            ell_max=ELL_MAX,
            simulations_per_round=[2000, 2000, 4000],
    ...
    if hmc is not None and sbi is not None:
        ...
        tri_path = OUTPUT_DIR / f"posterior1d_{probe_name}.png"
        g.export(str(tri_path))
        plt.close("all")
        print(f"  1-D posterior plot saved → {tri_path}")
