...
# Theory Cls outputs written to this folder
...
# =============================================================================
# PATH HELPERS  (theory Cls) — now per-probe
# =============================================================================
def theory_hmc_npz_path(probe):
    return THEORY_SBI_BASE / f'{probe}_linearized' / 'hmc_samples.npz'

def theory_sbi_npz_path(probe):
    return THEORY_SBI_BASE / f'{probe}_linearized_fisher_mdn5' / 'sbi_posterior_samples.npz'


# =============================================================================
# SAMPLE LOADERS  (theory Cls) — now take `probe` as an argument
# =============================================================================
...
def load_theory_hmc_mcsamples(probe, label='HMC / NUTS (theory)'):
    ...
    Keys: samples_{name}  for each param name in THEORY_PARAM_NAMES.
    """
    path = theory_hmc_npz_path(probe)
    if not path.exists():
        print(f'[SKIP] Theory HMC samples not found: {path}')
        return None
    ...
    print(f'  [theory:{probe}] HMC samples: {samples.shape}  '
          f'theta_ej_0={samples[:,0].mean():.3f}+/-{samples[:,0].std():.3f}  '
          f'nu={samples[:,1].mean():.3f}+/-{samples[:,1].std():.3f}')
...
def load_theory_sbi_mcsamples(probe, label='SBI / NPE+MDN (theory)'):
    ...
    in physical (theta) space — already back-transformed from Fisher basis.
    """
    path = theory_sbi_npz_path(probe)
    if not path.exists():
        print(f'[SKIP] Theory SBI samples not found: {path}')
        return None
    ...
    n_before = len(samples)
    samples  = _filter_theory_prior(samples)
    print(f'  [theory:{probe}] SBI samples: {len(samples)} / {n_before} inside prior  '
          f'theta_ej_0={samples[:,0].mean():.3f}+/-{samples[:,0].std():.3f}  '
          f'nu={samples[:,1].mean():.3f}+/-{samples[:,1].std():.3f}')
...
for probe in PROBES:
    ...
    # Theory contours are now loaded per-probe too
