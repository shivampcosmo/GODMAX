...
for probe_name in PROBES_TO_RUN:
    ...
    # ── 2D FISHER DEGENERACY DIAGNOSTIC ─────────────────────────────────────
    if len(PARAM_SPECS) == 2:
        J = theory_info["jacobian"]                    # shape (ndata, 2)
        fisher = J.T @ selected["precision"] @ J        # shape (2, 2)
        eigvals, eigvecs = np.linalg.eigh(fisher)
        cond = eigvals.max() / max(eigvals.min(), 1e-300)
        print(f"\n  Fisher matrix (2x2):\n{fisher}")
        print(f"  Fisher eigenvalues        : {eigvals}")
        print(f"  Fisher eigenvectors (cols): \n{eigvecs}")
        print(f"  Condition number          : {cond:.4e}")
        sigma_marginal = np.sqrt(np.diag(np.linalg.inv(fisher)))
        print(f"  Marginalized sigma (theta_ej_0, nu): {sigma_marginal}")
        # Degenerate direction = eigenvector of the SMALLEST eigenvalue
        degenerate_dir = eigvecs[:, np.argmin(eigvals)]
        print(f"  Most-constrained combination direction: {eigvecs[:, np.argmax(eigvals)]}")
        print(f"  Least-constrained (degenerate) direction: {degenerate_dir}")
