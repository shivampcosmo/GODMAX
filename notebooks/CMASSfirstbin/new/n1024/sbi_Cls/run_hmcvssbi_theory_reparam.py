...
# THEORY_PARAM_SPECS: physical basis (theta_ej_0, nu) -- what the GODMAX
# simulator / vector_fn / Jacobian are written in terms of.
# PARAM_SPECS: inference basis (theta_ej_0, phi) -- what HMC/SBI actually
...
FIDUCIAL_PATH = ensure_default_fiducial_product(
    DEFAULT_FIDUCIAL_PATH,
    param_specs=THEORY_PARAM_SPECS,   # physical basis: simulator needs real "nu_theta_ej_M"
    force=False,
)
print("Fiducial path    :", FIDUCIAL_PATH)

names     = [p.name      for p in PARAM_SPECS]   # phi basis — what gets sampled/saved
...
for probe_name in PROBES_TO_RUN:
    ...
    hmc_run_name  = f"{probe_name}_linearized_reparam"
    sbi_run_name  = f"{probe_name}_linearized_fisher_mdn5_reparam"
    ...
    # vector_fn_theory + theory_info are built in the PHYSICAL basis, matching
    # what run_hmc/run_sbi build internally from theory_param_specs.
    ...
    def vector_fn(theta):
        """Composed evaluator in the sampled (theta_ej_0, phi) basis."""
    ...
    # Raw physics sanity check: must be against vector_fn_theory / THEORY_PARAM_SPECS
    # (the physical fiducial), independent of theta_transform — same convention
    # used inside run_hmc / run_sbi.
    checks = validate_theory_vector(vector_fn_theory, selected, THEORY_PARAM_SPECS)
    print(f"  Theory validation: {checks}")

    # ── HMC ───────────────────────────────────────────────────────────────────
    if RUN_HMC:
        # Verify vector_fn actually changes with phi at the point HMC will use it
        test_fid = vector_fn(fiducial)
        test_phi = vector_fn(fiducial + np.array([0.0, 0.05]))
        print(f"  vector_fn change when nudging phi: {np.max(np.abs(test_phi - test_fid)):.4e}")
        run_hmc(
    ...
    if hmc is not None and sbi is not None:
        ...
        tri_path = OUTPUT_DIR / f"triangle_{probe_name}_reparam.png"
    ...
    if hmc is not None and sbi is not None:
        ...
        fig.tight_layout()
        marg_path = OUTPUT_DIR / f"marginals_1d_{probe_name}_reparam.png"
