...
CACHE_DIR = os.path.join(WORK_DIR, 'sample_vector_cache_clsnew')
VALIDATION_CSV = '/work/hdd/bdne/aacharya2/GODMAX/notebooks/CMASSfirstbin/new/n1024/validation_samples.csv'
VAL_CACHE_DIR  = os.path.join(WORK_DIR, 'validation_vector_cache_clsnew')
...
# ── Cl specs ──────────────────────────────────────────────────────────────────
# Fields used:
#   gal_sq    = δ_gal²  (mean-subtracted)          pixel window: pw²
#   kappa_sq  = κ²      (mean-subtracted)          pixel window: none
#   gkap_prod = δ_gal·κ (mean-subtracted product)  pixel window: pw¹
#   gal       = δ_gal                              pixel window: pw¹
#   ymap      = Compton-y                          pixel window: none
#   tau       = optical depth                      pixel window: none
#   kappa     = CMB lensing convergence            pixel window: none
CL_SPECS = [
    ('g2y',     'gal_sq',    'ymap'),
    ('g2tau',   'gal_sq',    'tau'),
    ('g2kappa', 'gal_sq',    'kappa'),
    ('k2g',     'kappa_sq',  'gal'),        # κ²g  — rotated degeneracy vs g²κ
    ('gky',     'gkap_prod', 'ymap'),       # gκy  — mixed 3-field bispectrum
    ('gy',      'gal',       'ymap'),
    ('gtau',    'gal',       'tau'),
    ('gkappa',  'gal',       'kappa'),
]
...
INDIVIDUAL_STATS = {'gy', 'gtau', 'gkappa', 'g2y', 'g2tau', 'g2kappa', 'k2g', 'gky'}
...
def extract_Cls(path):
    ...
    Ordering: [g2y | g2tau | g2kappa | k2g | gky | gy | gtau | gkappa],
              each N_ELL_BINS_ACTUAL long.

    New fields:
        kappa_sq  = (κ - mean_κ)²  mean-subtracted before squaring
        gkap_prod = δ_gal · κ      pixel-wise product, mean-subtracted

    Pixel window correction:
        gal       -> pw^1
        gal_sq    -> pw^2
        kappa_sq  -> none  (κ is a continuous field)
        gkap_prod -> pw^1  (one gal factor)
        ymap/tau/kappa -> none
    """
    ...
    mean_k      = float(np.mean(kmap_clean))
    delta_kappa = kmap_clean - mean_k                    # mean-subtracted κ

    kappa_sq    = delta_kappa ** 2
    kappa_sq   -= np.mean(kappa_sq)                      # mean-subtracted κ²

    gkap_prod   = delta_gal * delta_kappa                # δ_gal · δ_κ
    gkap_prod  -= np.mean(gkap_prod)                     # mean-subtracted
    ...
    field_map = {
        'gal':       (delta_gal,              pw1),
        'gal_sq':    (delta_sq,               pw2),
        'kappa_sq':  (kappa_sq,               None),   # continuous field
        'gkap_prod': (gkap_prod,              pw1),    # one gal factor
    ...
    for label, field_a, field_b in CL_SPECS:
        ...
        if label.startswith('g2') or label in ('k2g', 'gky'):
...
def train_one_statistic(args):
    ...
    try:
        ...
        if blocks is None:
            ...
            if name.startswith('g2') or name in ('k2g', 'gky'):
...
if __name__ == '__main__':
    ...
    print(f'STAT_MAP keys: {list(STAT_MAP.keys())}')
    ...
    if os.path.exists(x_train_path) and os.path.exists(theta_train_path):
        ...
        # Validate cached arrays have correct N_SUMMARY dimension
        if x_train.shape[1] != N_SUMMARY:
            ...
            print(f'  Cache valid: {x_train.shape}')
    else:
        x_train = None
        ...
        for csv_path, offset in CSV_FILES:
            ...
            for _, row in tqdm(df.iterrows(), total=len(df),
                ...
                if os.path.exists(cache_file):
                    ...
                    # Invalidate cache if wrong dimension (new CL_SPECS)
    ...
    print(f'\nGenerating round {NEXT_ROUND} proposals from ' f'{PROPOSAL_STAT} posterior...')
