...
def _get_noise_pkg():
    ...
    for field in ('nl_yy', 'nl_tautau', 'nl_kk'):
        ...
        interped[:2]    = 0.0   # zero monopole and dipole
        pkg[field]      = interped   # plain nl, no pixwin scaling
...
def _add_noise_to_maps(ymap, tmap, kmap, path: str) -> tuple:
    """
    Add a deterministic noise realization to y, tau, kappa maps.
    Seed is derived from path so the same simulation always gets
    the same noise draw. Plain nl passed to synfast (no pixwin scaling)
    — consistent with extract_Cls dividing cross-spectra by pw1 only.
    """
    pkg  = _get_noise_pkg()
...
def extract_Cls(path, add_noise=ADD_SURVEY_NOISE):
    ...
    If add_noise=True, adds a deterministic per-path noise realization to
    y, tau, and kappa maps before computing power spectra (plain nl, no pixwin
    scaling — consistent with dividing cross-spectra by pw^1 for gal only).
    ...
    # ── Add survey noise realizations before power spectrum measurement ────────
    if add_noise:
...
if __name__ == '__main__':
    ...
    # Pre-load noise package at startup so worker diagnostics print once
    if ADD_SURVEY_NOISE:
        _get_noise_pkg()
