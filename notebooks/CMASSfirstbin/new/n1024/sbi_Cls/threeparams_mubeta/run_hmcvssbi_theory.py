# threeparams_mubeta/run_hmcvssbi_theory_3params.py
# ──────────────────────────────────────────────────────────────────────────────
# Same as run_hmcvssbi_theory.py but with a third parameter: mu_beta
# Outputs go to sbi_Cls/threeparams_mubeta/outputs/theory_sbi/
# ──────────────────────────────────────────────────────────────────────────────
...
THIS_DIR         = pathlib.Path(__file__).parent.resolve()          # sbi_Cls/threeparams/
...
print("threeparams dir  :", THIS_DIR)
...
# =============================================================================
# FIDUCIAL PRODUCT
# fiducial at mu_beta=0.5 is identical physics to the 2-param fiducial
# =============================================================================
...
for probe_name in PROBES_TO_RUN:
    ...
    print(f"\n  Jacobian shape: {theory_info['jacobian'].shape}"
          f"  (expect ({selected['data_vector'].shape[0]}, {len(PARAM_SPECS)}))")
    ...
    # ── Fisher matrix diagnostic (3x3) ───────────────────────────────────────
    J         = theory_info["jacobian"]          # shape (N_ell, 3)
    chol_diag = np.diag(selected["chol"])
    W         = J / chol_diag[:, None]           # whitened Jacobian
    F         = W.T @ W                           # 3x3 Fisher matrix
    print(f"\n  Fisher matrix (3x3):")
    print(f"  {F}")
    try:
        F_inv = np.linalg.inv(F)
        print(f"  Marginal sigmas:")
        for i, spec in enumerate(PARAM_SPECS):
            print(f"    sigma({spec.name}) = {np.sqrt(F_inv[i,i]):.4f}"
                  f"  (prior width = {spec.prior_max - spec.prior_min:.1f})")
        # Correlation matrix
        D = np.sqrt(np.diag(F_inv))
        corr = F_inv / np.outer(D, D)
        print(f"  Correlation matrix:")
        for i, si in enumerate(PARAM_SPECS):
            row = "  ".join(f"{corr[i,j]:+.4f}" for j in range(len(PARAM_SPECS)))
