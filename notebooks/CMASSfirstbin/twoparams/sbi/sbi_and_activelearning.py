...
def extract_moments(path):
    """
    Extract summary statistics from all map pickle files in path.

    Vector layout : 9 statistics per scale x 5 scales = 45 elements total.
    Within each scale block:
      idx 0: <ggy>   (3pt cross, y tracer)
      idx 1: <ggt>   (3pt cross, tau tracer)
      idx 2: <ggk>   (3pt cross, kappa tracer)
      idx 3: <gy>    (2pt cross, y tracer)
      idx 4: <gt>    (2pt cross, tau tracer)
      idx 5: <gk>    (2pt cross, kappa tracer)
      idx 6: <y2>    auto-moment, y tracer)
      idx 7: <tau2>  (auto-moment, tau tracer)
      idx 8: <kappa2>    (auto-moment, kappa tracer)

    The auto-moments <X2>(theta) are shape-sensitive: their ratio across scales
    encodes the radial profile slope, which is directly sensitive to mu_beta.
    """
        ...
        with open(f, 'rb') as h:
            ...
            for chunk_idx in mock_gals_dict:
                ...
                if gal_data.size > 0:
                    ...
                    if n_bad > 0:
                        print(f"  [WARN] Dropping {n_bad} galaxies with "
                              f"non-finite coordinates in chunk {chunk_idx}")
    ...
    for th in SCALES:
        ...
        # 3-point cross-moments <ggX>
        vec.extend([np.mean(gs**2 * ys),
                    np.mean(gs**2 * ts),
                    np.mean(gs**2 * ks)])

        # 2-point cross-moments <gX>
        vec.extend([np.mean(gs * ys),
                    np.mean(gs * ts),
                    np.mean(gs * ks)])

        # Auto-moments <X2> --> directly encodes profile shape / slope
        vec.extend([np.mean(ys**2),
                    np.mean(ts**2),
                    np.mean(ks**2)])

    return np.array(vec, dtype=np.float32)   # length 45
...
# =============================================================================
# stat_map: 9 statistics per scale x 5 scales = 45 elements.
# Within each scale block of 9:
#   offsets 0,1,2 --> ggy, ggt, ggk ( 3pt cross)
#   offsets 3,4,5 --> gy,  gt,  gk   (2pt cross)
#   offsets 6,7,8 --> yy, tt,  kk (auto-moments)
#
# Scale 0 (4  arcmin): indices  0-8
# Scale 1 (8  arcmin): indices  9-17
# Scale 2 (16 arcmin): indices 18-26
# Scale 3 (32 arcmin): indices 27-35
# Scale 4 (64 arcmin): indices 36-44
...
next_theta = sample_ensemble_direct(joint_posterior, xo_joint_norm, n_samples=30)
...
out_csv = f'round{NEXT_ROUND}_samples.csv'
pd.DataFrame(next_theta, columns=['theta_ej_0', 'mu_beta']).to_csv(out_csv, index_label='sample_id')
...
print(f"  mu_beta:    mean={next_theta[:,1].mean():.3f}, "
