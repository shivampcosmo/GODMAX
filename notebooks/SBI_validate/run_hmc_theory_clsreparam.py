...
def run_hmc(
    ...
    param_specs defines independent Uniform(prior_min, prior_max) priors,
    with no Jacobian correction, regardless of what the two parameters
    physically represent.

    theory_param_specs / theta_transform only exist because the GODMAX
    simulator's equations are written in terms of specific physical
    quantities (e.g. nu). If you sample in a different basis (e.g. phi),
    the sampled value must be converted to nu before being handed to the
    simulator -- otherwise it gets written directly into the "nu" slot as
    if it were nu itself. This is a bookkeeping necessity, not a prior or
    Jacobian correction: theta_transform performs a plain deterministic
    change of variables and registers no numpyro.factor.
    """
    ...
    validation = validate_theory_vector(vector_fn, selected, theory_specs)
    ...
    def model():
        ...
        theta_physical = theta_transform(theta) if theta_transform is not None else theta
        mu = vector_fn(theta_physical)
        resid = obs - mu
