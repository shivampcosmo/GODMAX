...
if _np_trapezoid is None:  # NumPy < 2.0
...
def nbar_to_nz_lens(nbar_comoving, z_array, cosmo_jax):
    ...
    norm = _np_trapezoid(unnorm, z_array)
    if norm > 0:
        return unnorm / norm
    return unnorm
...
def update_nz_from_mock_catalog(mock_gals, analysis_dict, zarray_lens, cosmo_jax,
    ...
    norm = _np_trapezoid(nz_raw, zarray_lens)
...
def generate_maps(ra_all, dec_all, z_all, M200c_all, vlos_all,
    ...
    for i in range(num_chunks):
        ...
        if result:
            ...
            if hasattr(mock_map, 'rhommap_final'):
                rhom = np.nan_to_num(mock_map.rhommap_final)
                map_rhom_dmb += rhom
                map_rhom += rhom
            if hasattr(mock_map, 'rhom_dmo_map_final'):
    ...
    saved_data = {
        ...
        'map_metadata': {
            ...
            'max_paint_R200c_factor': 8.0,
            'nside': int(nside),
            'field_accumulation_version': 'single_add_v2',
            'field_accumulation_note': 'Each generated field is accumulated exactly once per chunk.',
        },
    }
    ...
    for iz in range(nz_bins):
        ...
        for im in range(nM_bins):
            ...
            if mask_M.sum() > 0:
                ...
                ndens_bin = _np_trapezoid(dndlnM_z[mask_M], np.log(M_th[mask_M]))
...
def compute_Cl_gg_1h_2h(Cls_test):
    ...
    for jl, ell in enumerate(ell_array):
        ...
        Cl_1h[jl] = _np_trapezoid(integrand_1h, z_arr)
        Cl_2h[jl] = _np_trapezoid(integrand_2h, z_arr)
...
def compute_hod_shot_noise_Cl(Cls_test, jb=0):
    ...
    Cl_shot_hod = float(_np_trapezoid(integrand, z_arr))
