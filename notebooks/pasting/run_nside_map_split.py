...
for i in tqdm(range(num_chunks)):
    ...
    if map_type == 'tsz':
        ...
        map_test += np.array(np.nan_to_num(mock_map_test.taumap_final), dtype=np.float32)
    elif map_type == 'ksz':
        map_test += np.array(np.nan_to_num(mock_map_test.kszmap_final), dtype=np.float32)
    else:
        print('map_type not supported')
