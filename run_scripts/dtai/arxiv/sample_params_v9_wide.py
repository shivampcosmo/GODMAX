...
with open(abs_path_params + '/DESxACT/priors_v2_wide.yaml', 'r') as file:
    data = yaml.safe_load(file)
...
dill.dump(trace, open(save_chain_dir + f'mcmc_v9wide_probe_{probe}_modelmatter_{model_matter}_deproj_{deproj}_samples_{num_samples}_warmup_{num_warmup}_num_chains_{num_chains*n_parallel}_treedepth_{max_tree_depth}_gtysc_{use_gty_scale_cuts}_Y3xipmsc_{use_xipm_Y3_scale_cuts}.pkl', 'wb'))
