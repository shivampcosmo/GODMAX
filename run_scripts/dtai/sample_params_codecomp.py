...
df_measure = pk.load(open(f'{save_DV_dir}/DESxACT_CODECOMP_SIMDV_gty_xip_xim_DV_ilc_SZ_deproj_cib_cibdBeta_1.7_10.7_yy.pk', 'rb'))
...
with open(abs_path_params + '/DESxACT/priors_v3_codecomp.yaml', 'r') as file:
    data = yaml.safe_load(file)
...
other_params_vary_names = ['A_IA', 'eta_IA']
...
dill.dump(trace, open(save_chain_dir + f'mcmc_codecomp_v11fixbaryon_probe_{probe}_modelmatter_{model_matter}_deproj_{deproj}_samples_{num_samples}_warmup_{num_warmup}_num_chains_{num_chains*n_parallel}_treedepth_{max_tree_depth}_gtysc_{use_gty_scale_cuts}_Y3xipmsc_{use_xipm_Y3_scale_cuts}.pkl', 'wb'))
