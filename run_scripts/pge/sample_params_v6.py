...
def parse_args():
    ...
    parser.add_argument('--bao_prior', type=bool, default=0,
                        help='Use BAO prior')
    parser.add_argument('--init_strategy', type=str, default="value",
...
if model_matter == 'halofit':
    savefname_out = save_chain_dir + f'{probes_forecast_all_str}/mcmc_v6_{probes_forecast_all_str}_scval_{sc_val}_samples_{num_samples}_warmup_{num_warmup}_num_chains_{num_chains*n_parallel}_treedepth_{max_tree_depth}_wbaoprior_{wbao_prior}_halofit.pkl'
else:
    savefname_out = save_chain_dir + f'{probes_forecast_all_str}/mcmc_v6_{probes_forecast_all_str}_scval_{sc_val}_samples_{num_samples}_warmup_{num_warmup}_num_chains_{num_chains*n_parallel}_treedepth_{max_tree_depth}_wbaoprior_{wbao_prior}.pkl'
# if wbao_prior:
...
if run_this_script:
    ...
    df = pk.load(open(abs_path_data + '/pge/DV_fid_Cl_Pk_cov_mthresh_hod_v6_symb_dataconsist.pk', 'rb'))
