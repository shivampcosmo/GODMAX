...
class Profiles(base_class):
    ...
    def _compute_zeta_mat_with_scan(self):
        """
        Memory-efficient computation of zeta_mat using jax.lax.scan.
        This processes radii sequentially to reduce peak memory usage.
        """
        from jax import lax

        # Process over nz and nM with vmap, but use scan over nr
        def compute_zeta_for_r(carry, jr):
            # Compute zeta for all (jz, jM) combinations at this radius
            zeta_r = get_vmapped_func(self.get_zeta, 2)(jnp.array([jr]), jnp.arange(self.nz), jnp.arange(self.nM))[0].T
            return carry, zeta_r

        # Use scan to iterate over radii
        _, zeta_mat = lax.scan(compute_zeta_for_r, None, jnp.arange(self.nr))
        # zeta_mat shape: (nr, nz, nM)
        return zeta_mat

    @partial(jit, static_argnums=(0,))
    def _compute_zeta_mat_chunked(self, chunk_size=None):
        """
        Chunked computation of zeta_mat to control memory usage.
        Processes the mass dimension in chunks.
        """
        if chunk_size is None:
            chunk_size = max(1, self.nM // 4)  # Default to 4 chunks

        # Process in chunks over nM dimension
        n_chunks = (self.nM + chunk_size - 1) // chunk_size
        zeta_chunks = []

        for i in range(n_chunks):
            start_idx = i * chunk_size
            end_idx = min((i + 1) * chunk_size, self.nM)
            chunk_indices = jnp.arange(start_idx, end_idx)

            # Compute for this chunk
            zeta_chunk = get_vmapped_func(self.get_zeta, 3)(
                jnp.arange(self.nr),
                jnp.arange(self.nz),
                chunk_indices
            ).T
            zeta_chunks.append(zeta_chunk)

        # Concatenate along mass dimension
        return jnp.concatenate(zeta_chunks, axis=2)
    ...
    def run_clm_calc(self):
        ...
        # Pre-calculate zeta values using chunked/sequential processing to reduce memory
        # Option 1: Use scan over radii to reduce memory (RECOMMENDED for sampling)
        if getattr(self, 'use_scan_zeta', False):
            self.zeta_mat = self._compute_zeta_mat_with_scan()
        # Option 2: Use vmap with reduced resolution (faster but more memory)
        else:
    ...
    def get_zeta(self, jr, jz, jM, r_array_here=None):
        ...
        # Use adaptive resolution based on use_scan_zeta flag
        n_zeta_points = getattr(self, 'n_zeta_points', 8)
        if getattr(self, 'use_scan_zeta', False):
            # Use fewer points during sampling to reduce memory
            n_zeta_points = min(n_zeta_points, 16)

        zeta_array = jnp.linspace(0.5, 1.5, n_zeta_points)
        value_out = vmap(zeta_equation)(zeta_array)
        zeta = jnp.interp(0.0, value_out, zeta_array)
        return zeta

    @partial(jit, static_argnums=(0,))
    def get_zeta_optimized(self, jr, jz, jM, r_array_here=None):
        '''
        More memory-efficient version using bisection-like approach
        instead of vmapping over many trial values.
        '''
        if r_array_here is None:
            ri = self.r_array[jr]
        else:
        ...
        # Coarse search with only 8 points instead of 32
        zeta_array_coarse = jnp.linspace(0.5, 1.5, 8)
        value_out_coarse = vmap(zeta_equation)(zeta_array_coarse)
        zeta_coarse = jnp.interp(0.0, value_out_coarse, zeta_array_coarse)

        # Optional: Refine with a few more points around the coarse solution
        # zeta_array_fine = jnp.linspace(zeta_coarse - 0.1, zeta_coarse + 0.1, 8)
        # value_out_fine = vmap(zeta_equation)(zeta_array_fine)
        # zeta = jnp.interp(0.0, value_out_fine, zeta_array_fine)
