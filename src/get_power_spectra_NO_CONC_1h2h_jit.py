    ...
    def __init__(
        ...
        if analysis_dict['do_sheary']:
            if self.get_sep_1h2h:
                ...
                self.Cl_kappa_y_tot_mat = self.Cl_kappa_y_1h_mat + self.Cl_kappa_y_2h_mat
        ...
        if analysis_dict.get('do_yy', False):
            if self.get_sep_1h2h:
                ...
                self.Cl_y_y_tot_mat = vmap(self.get_Cl_y_y_1h)(jnp.arange(self.nell)) + vmap(self.get_Cl_y_y_2h)(jnp.arange(self.nell))
                if verbose_time:
                    print('Time for computing Cl_y_y_tot_mat: ', time.time() - ti)
                    ti = time.time()
        ...
        if analysis_dict['do_shear2pt']:
            if self.get_sep_1h2h:
                ...
                if verbose_time:
                    print('Time for computing Cl_kappa_kappa_tot_mat: ', time.time() - ti)
                    # print('Total time for computing all Cls: ', time.time() - t0)
                    ti = time.time()
            ...
            if self.calc_nfw_only:
                if self.get_sep_1h2h:
                    ...
                    if verbose_time:
                        print('Time for computing Cl_kappa_kappa_nfw_tot_mat: ', time.time() - ti)
                        print('Total time for computing all Cls: ', time.time() - t0)
                    # ti = time.time()                
