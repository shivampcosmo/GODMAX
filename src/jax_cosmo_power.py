...
def sigmasqr(cosmo, R, transfer_fn, kmin=0.0001, kmax=500.0, divmax=5, **kwargs):
    ...
    y = romb(int_sigma, np.log10(kmin), np.log10(kmax), divmax=divmax)
...
def halofit_parameters(cosmo, a, transfer_fn=tklib.Eisenstein_Hu, num_points=64):
    ...
    def R_nl(a):
        ...
        sigma = simps(int_sigma, np.log(1e-4), np.log(1e4), num_points)
    ...
    res = simps(integrand, np.log(1e-4), np.log(5e2), num_points)

    n_eff = res[0] - 3.0
    C = res[0] ** 2 + res[1]
    # print(k_nl.shape, n_eff.shape, C.shape)

    # return k_nl, n_eff, C
    return np.array([k_nl, n_eff[0], C[0]])



def halofit(cosmo, k, a, transfer_fn=tklib.Eisenstein_Hu, pklin=None, hfit_params=None, a_array=None, prescription="takahashi2012"):
    ...
    if pklin is None:
        ...
        ind_a = np.argmin(np.abs(a_array - a))
        pklin = pklin[:,ind_a]

    if hfit_params is None:
        # Compute non linear scale, effective spectral index and curvature
        hfit_params = halofit_parameters(cosmo, a, transfer_fn)
        k_nl, n, C = hfit_params[0], hfit_params[1], hfit_params[2]
    else:
        ind_a = np.argmin(np.abs(a_array - a))
        k_nl = hfit_params[0,ind_a]
        n = hfit_params[1,ind_a]
        C = hfit_params[2,ind_a]
...
def nonlinear_matter_power(
    cosmo, k, a=1.0, pklin=None, hfit_params=None, a_array=None, nonlinear_fn=halofit, transfer_fn=tklib.Eisenstein_Hu
):
    """Computes the non-linear matter power spectrum.

    This function is just a wrapper over several nonlinear power spectra.
    """
    return nonlinear_fn(cosmo, k, a, transfer_fn=transfer_fn, pklin=pklin, hfit_params=hfit_params, a_array=a_array)