...
class mcfit(object):
    ...
    def __init__(self, x, MK, q, nx=None, lowring=False, xy=1, backend='jax'):
        if backend == 'numpy':
            self.np = numpy
        ...
        self.q = q
        # self.N = N
        self.N = 2**(math.ceil(math.log2(self.Nin * 2)))
    ...
    def _setup(self):
        ...
        #     self.N = 2**self.folds
        # else:
        #     folds = jnp.int32(jnp.ceil(jnp.log2(self.Nin * self.N.imag)))
        #     self.N = 2**folds
        
        # if self.N < self.Nin:
        #     raise ValueError(f"convolution size {self.N} must not be smaller than "
        #                      f"the input size {self.Nin}")
        # True branch function
        def true_branch(_):
            lnxy = Delta / jnp.pi * jnp.angle(self.MK(self.q + 1j * jnp.pi / Delta))
            return lnxy

        # False branch function
        def false_branch(_):
            return jnp.log(self.xy)
        
        if self.lowring:
            lnxy = jax.lax.cond(self.N % 2 == 0, true_branch, false_branch, None)
        ...
        self._u *= self.np.exp(-2j * jnp.pi * lnxy / self.N / Delta * m)
    ...
    def _pad(self, a, axis_orig, extrap, out):
       ...
       # if a.shape[axis] == self.N:
        ...
        # def false_branch(_):
        axis = axis_orig%a.ndim # to fix the indexing below with axis+1
        ...
        # return jax.lax.cond(a.shape[axis_orig] == self.N, true_branch, false_branch, None)
    ...
    #         def pad_with_zeros(pad_width, shape):
    #             pad_shape = list(shape)
    #             pad_shape[axis] = pad_width
    #             return self.np.zeros(pad_shape)

    #         def pad_const(end, pad_width):
    #             repeat_shape = [1] * end.ndim
    #             repeat_shape[axis] = pad_width
    #             return self.np.tile(end, repeat_shape)

    #         def pad_power_law(end, ratio, pad_width, is_left):
    #             exponent = self.np.arange(1, pad_width + 1)
    #             if is_left:
    #                 exponent = -exponent
    #             return end * (ratio ** exponent.reshape((-1,) + (1,) * (end.ndim - 1)))

    #         # Extract the endpoints and ratios for extrapolation
    #         end_left = a.take(0, axis=axis)
    #         ratio_left = a.take(1, axis=axis) / end_left
    #         end_right = a.take(-1, axis=axis)
    #         ratio_right = end_right / a.take(-2, axis=axis)

    #         def apply_extrap(end, ratio, pad_width, extrap, is_left, pad_with_zeros, pad_const, pad_power_law):
    #             def true_branch_extrap(_):
    #                 return jnp.array([]).reshape((0,) + end.shape)
                
    #             def false_branch_extrap(_):
    #                 # if pad_width <= 0:
    #                 #     return jnp.array([]).reshape((0,) + end.shape)
    #                 if isinstance(extrap, bool):
    #                     if extrap:
    #                         # Power-law extrapolation
    #                         return pad_power_law(end, ratio, pad_width, is_left)
    #                     else:
    #                         # Zero padding
    #                         return pad_with_zeros(pad_width, end.shape)
    #                 elif extrap == 'const':
    #                     # Constant padding
    #                     return pad_const(end, pad_width)
    #                 else:
    #                     raise ValueError(f"Unsupported extrapolation method: {extrap}")

    #             return jax.lax.cond(pad_width <= 0, true_branch_extrap, false_branch_extrap, None)

    #         # Apply extrapolation based on `extrap`
    #         _a = apply_extrap(end_left, ratio_left, _Npad, extrap, True, pad_with_zeros, pad_const, pad_power_law)
    #         a_ = apply_extrap(end_right, ratio_right, Npad_, extrap, False, pad_with_zeros, pad_const, pad_power_law)
